...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/dist/css/admin_stock.css?v=2.0">
		...
		<div id="page-content-wrapper">
			...
			<div class="content-container">
				<div class="page-header">
					...
					<p class="page-desc">Manage footwear inventory by color and
						size variants</p>
				</div>

				<div class="card-box mb-4">
					<form name="searchForm"
						action="${pageContext.request.contextPath}/admin/product/stock_list"
						method="get">
						<div class="search-main-row">
							<div class="search-group-input">
								<div class="search-item">
									<label class="search-label">카테고리</label> <select
										class="form-select premium-select" name="cateCode">
										<option value="">전체 카테고리</option>
										<c:forEach var="vo" items="${categoryList}">
											<option value="${vo.cateCode}"
												${cateCode == vo.cateCode ? "selected" : ""}>
												${vo.cateName}</option>
										</c:forEach>
									</select>
								</div>

								<div class="search-item flex-grow-item">
									<label class="search-label">상품명 검색</label> <input type="text"
										class="form-control premium-input" name="keyword"
										value="${keyword}" placeholder="상품명을 입력해 주세요">
								</div>
							</div>
				...
				<div class="card-box">
					<div class="table-responsive">
						<table class="table table-hover align-middle">
							<thead class="table-light">
								<tr>
									<th width="80" class="text-center">No / <input
										type="checkbox" id="checkAll" class="form-check-input"
							...
							<tbody>
								<c:forEach var="dto" items="${list}" varStatus="status">
									<tr>
										<td class="text-center">${dataCount - (page-1) * 10 - status.index}
											<input type="hidden" name="prodId" value="${dto.prodId}">
											<input type="checkbox" name="optIds" value="${dto.optId}"
										...
										<td class="text-center fw-bold text-secondary">${dto.prodSize}
											mm</td>

										<td
											class="text-center fw-bold ${dto.stockQty == 0 ? 'text-danger' : ''}">
										...
										<td class="text-center small text-muted">${not empty dto.regDate ? dto.regDate.substring(0, 10) : '-'}
										</td>
								...
								<c:if test="${dataCount == 0}">
									<tr>
										<td colspan="8" class="text-center p-5">등록된 재고 데이터가 없습니다.</td>
									</tr>
								</c:if>
							</tbody>
						</table>
					...
					<div class="btn-group-custom mt-3 text-end">
						<button type="button" class="btn btn-luxury btn-orange"
							onclick="openBulkStockModal()">SAVE CHANGES</button>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content" style="border-radius: 2px;">
				...
				<div class="modal-body p-4">
					<div class="mb-3">
						<label class="form-label text-muted small fw-bold">제품명 /
							사이즈</label> <input type="text" id="modalProdName" class="form-control"
							readonly style="background: #f8f9fa;">
					</div>

					<div class="mb-3" id="currentStockArea">
						<label class="form-label text-muted small fw-bold">현재 남은
							재고량</label> <input type="text" id="modalCurrentStock"
					...
					<div class="row">
						...
						<div class="col-md-6 mb-3">
							...
							<input type="number" id="modalChangeQty" class="form-control"
								placeholder="+/- 숫자 입력">
							<div class="form-text" style="font-size: 11px;">* 감소 시 -숫자
								입력 (예: -5)</div>
						</div>
					</div>

					<input type="hidden" id="modalOptId"> <input type="hidden"
						id="modalProdId"> <input type="hidden" id="modalMode"
						value="single">
				</div>
				<div class="modal-footer border-0 pt-0">
					<button type="button" class="btn btn-dark w-100"
						onclick="updateStockSubmit()">저장 (Update)</button>
				</div>
			</div>
		</div>
	</div>
