        ...
        <div class="chat-container">
            <div class="chat-header-modern">
                <div>
                    ...
                    <div class="chat-meta-info">NO. ${dto.num} &nbsp;|&nbsp; ${dto.reg_date}</div>
                </div>
                <div>
                    <c:choose>
            ...
            <div class="chat-body-modern">
                        ...
                        <c:if test="${not empty dto.saveFilename}">
                            <div style="margin-top:15px;">
                                <div class="file-attach">
                                    ...
                                    <a href="${pageContext.request.contextPath}/cs/download?num=${dto.num}">${dto.originalFilename}</a>
                                </div>
                            </div>
                        </c:if>
                    </div>
                ...
                <c:if test="${not empty dto.replyContent}">
                    <div class="chat-msg admin">
                        <div class="msg-profile">
                            <span class="admin-badge">CS TEAM</span>
                            <span class="admin-name">Salomon Support</span>
                        </div>
                        <div class="msg-bubble">
                            ${dto.replyContent}
                        </div>
                ...
                <c:if test="${empty dto.replyContent}">
                     <div style="text-align: center; color: #aaa; font-size: 13px; margin-top: 30px;">
                        담당자가 확인 후 순차적으로 답변을 드립니다.
                     </div>
                </c:if>
            </div>

            <div class="btn-area" style="padding-bottom: 40px; background: #fff;">
                <button type="button" class="btn-cancel" onclick="location.href='${pageContext.request.contextPath}/cs/list?${query}'">목록으로</button>
                
                <c:if test="${sessionScope.member.userId == dto.userId}">
                    <button type="button" class="btn-cancel" style="color:#d9534f; border-color:#d9534f;" 
                            onclick="deleteBoard('${dto.num}', '${query}')">삭제</button>
                </c:if>
            </div>
        </div>
    </div>
